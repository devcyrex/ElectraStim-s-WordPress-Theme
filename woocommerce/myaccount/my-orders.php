						...
						<?php
							...
							if ( $actions ) {
								foreach ( $actions as $key => $action ) {
									echo '<a href="' . esc_url( $action['url'] ) . '" class="button button btn btn-primary btn-sm ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
								}
							}
						?>
					</td>
...
<?php else:
	...
	?>
	<h4 class="no-entries"><?php echo sprintf(__("You don't have any order's yet! <a href=\"{$shop_link}\">Go shopping &raquo;</a>", 'aurum'), get_permalink(wc_get_page_id('shop'))); ?></h4>
<?php endif; ?>
